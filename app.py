...
def create_specialized_agents():
    """Create all specialized agents for different analysis types"""
    try:
        ollama_llm = LLM(
...
def main():
    ...
    with st.sidebar:
        ...
        if ollama_status:
            ...
            st.info("Install Ollama with llama3.2:3b for expert analysis")
        ...
        with st.expander("How to use"):
            ...
            3. Select analysis type (if Ollama available)
            4. Get expert insights!
            """)
        
        with st.expander("Supported animals"):
            st.markdown("""
            - Domestic cats and dogs
            - Wild cats (lion, tiger, cheetah)
            - Birds (eagle, peacock, owl)
            - Large mammals (elephant, bear)
            - Primates (gorilla, chimpanzee)
            - Marine life (sharks, fish)
            """)
